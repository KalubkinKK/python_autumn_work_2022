...
#  форма должна отправляться на сервер где сохраняться в БД.
...
app.config["SQLALCHEMY_DATABASE_URI"] = 'postgresql://postgres:1234567@localhost:5432'
db.init_app(app)
...
class User(db.Model):
    ...
    online = db.Column(db.String, nullable=True)
